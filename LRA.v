...
Definition LRA_literal dim := [eqType of bool * rat^o ^ dim].
(*
b, (c_1, ..., c_d) =>
...
Definition LRA_interpret_af dim (I f : rat ^ dim) :=
  (\sum_(i < dim) f i * I i)%R.
...
Lemma LRA_interpret_af_add dim (I f1 f2 : rat ^ dim) :
  (LRA_interpret_af I (f1 + f2) =
   LRA_interpret_af I f1 + LRA_interpret_af I f2)%R.
Proof.
by rewrite /LRA_interpret_af -big_split /=;
  apply/eq_bigr => i _; rewrite ffunE mulrDl.
Qed.

Lemma LRA_interpret_af_opp dim (I f : rat ^ dim) :
  (LRA_interpret_af I (- f) = - LRA_interpret_af I f)%R.
Proof.
by rewrite /LRA_interpret_af (big_endo _ (@opprD _)) //;
  apply/eq_bigr => /= i _; rewrite ffunE mulNr.
Qed.
...
Fixpoint QFLRA_interpret_formula
  ...
  match f with
    ...
    | QFLRA_imply f1 f2 => QFLRA_interpret_formula I f1 ==>
                           QFLRA_interpret_formula I f2
    | QFLRA_leq t       => (0 <= \sum_(i < dim) t i * I i)%R
  end.
...
by apply eq_bigr => i _; rewrite ffunE mulNr opprK.
Qed.
...
by apply eq_bigr => i _; rewrite ffunE mulNr.
Qed.
...
have af_decomp (l1 l2 : LRA_literal dim.+1) :
  ...
  by rewrite !mulrA -mulNr -mulrDl !ffunE.
...
- case => x H; apply/andP; split.
  ...
  + apply/allP => l /allpairsP [] [] /= l1 l2 /= [].
    rewrite !mem_filter => //= /andP [H0] /(allP H) H1
                               /andP [H2] /(allP H) H3 ->.
    move: H0 H2 H1 H3.
    rewrite /LRA_interpret_literal /= !LRA_af_recl cons_tuple_eq1
...
Fixpoint Fourier_Motzkin dim (f : LRA_formula dim) : QFLRA_formula dim :=
  match f with
    ...
    | LRA_leq t1 t2 =>
      QFLRA_leq
        [ffun i => ((LRA_af_of_term t2 i)%:Z - (LRA_af_of_term t1 i)%:Z)%:Q]%R
  end.
